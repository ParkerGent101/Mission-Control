...
// NOTE: the war-planet viz (DonutChart) + its seeded-PRNG helpers (hashStr/mulberry32/
// todayStr) now live in static/planet.jsx (true-3D three.js globe, single source of truth,
// loaded after this file in index.html). Both the Finance and Health cards still render
// <DonutChart ... war /> with the same prop contract.
...
const CalendarCard = ({ cardProps = {} } = {}) => {
  ...
  const SF = (window.MCViz || {}).ScopeFrameFluid;   // responsive scanner-scope frame for the month grid
  ...
  return (
    ...
    >
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:3}}>
        ...
        {cells.map((d,i)=>{
          ...
          return (
            <div key={i} onClick={()=>setSelectedDay(s=>s===ds?null:ds)} title={evs.map(e=>e.title).join(', ')} style={{
              ...
              {/* event-density micro-bars: one slim type-coloured bar per event (capped), reads as an activity meter */}
              <div style={{display:'flex',gap:2,flexWrap:'nowrap',justifyContent:'center',alignItems:'flex-end',marginTop:4,height:9}}>
                {evs.slice(0,6).map((e,j)=>(<span key={j} title={e.title} style={{width:3,height:9,borderRadius:1,background:cv(e.type),boxShadow:`0 0 3px ${cv(e.type)}`}}/>))}
                {evs.length>6 && <span className="mono" style={{fontSize:8,color:'var(--ink-4)',marginLeft:1}}>+{evs.length-6}</span>}
              </div>
            </div>
          );
        })}
        ...
        return SF ? <SF accent="var(--accent-2)">{grid}</SF> : grid;
      })()}
